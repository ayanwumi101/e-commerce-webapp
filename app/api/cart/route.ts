...
export async function GET() {
  try {
    ...
    // Define the type for cart item with calculated fields
    interface CartItemWithTotals {
      id: string
      productId: string
      size: string
      qty: number
    ...
    // Calculate totals
    const items: CartItemWithTotals[] = cartItems.map((item) => {
      const discountedPrice = item.discount ? item.price * (1 - item.discount / 100) : item.price
      return {
        ...item,
        discountedPrice,
        itemTotal: discountedPrice * item.qty,
      } as CartItemWithTotals
    })
