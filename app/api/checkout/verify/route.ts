...
export async function POST(request: NextRequest) {
  try {
    ...
    // Send emails in parallel and capture results
    const [customerEmailSent, ownerEmailSent] = await Promise.all([
      sendOrderConfirmationEmail(
        ...
        orderData.shippingAddress ? 
          `${orderData.shippingAddress.street}, ${orderData.shippingAddress.city}${orderData.shippingAddress.region ? ', ' + orderData.shippingAddress.region : ''}, ${orderData.shippingAddress.country}` :
          "N/A"
      ),
    ...
    console.log("[v0] Email send results - Customer:", customerEmailSent, "Owner:", ownerEmailSent)
